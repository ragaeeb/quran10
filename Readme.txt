...
CREATE TABLE mentions (id INTEGER PRIMARY KEY, from_id INTEGER REFERENCES individuals(id) ON DELETE CASCADE ON UPDATE CASCADE, target INTEGER NOT NULL REFERENCES individuals(id) ON DELETE CASCADE ON UPDATE CASCADE, body TEXT NOT NULL, reference TEXT, points INTEGER, CHECK(body <> '' AND reference <> ''));
